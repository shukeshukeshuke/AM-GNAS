...
def extract_multi_hop_neighbors(g, K = 3, max_edge_attr_num = 1, max_hop_num = 5,
                                max_edge_type = 1, max_edge_count = 20000, max_distance_count = 1000, kernel = 'spd'):
    ...
    edge_attr = (torch.ones([g.number_of_edges()]) * 2).long()
    adj = g.adjacency_matrix_scipy()

    edge_attr_adj  = g.adjacency_matrix_scipy(transpose=False, return_edge_ids=True)
    edge_attr_adj  = sp.coo_matrix(edge_attr_adj)
    edge_attr_adj .data = sp.coo_matrix(edge_attr).data
    edge_attr_adj  = torch.from_numpy(edge_attr_adj .toarray()).long()
    ...
    edge_attr = torch.cat(edge_attr_list, dim=-1).float()  # E * K
    if K > 1:
    ...
    g.edata.pop('feat')
    g = dgl.add_edges(g, edge_index[0], edge_index[1], {'edge_attr': edge_attr})
    g.ndata['pe_attr']    = pe_attr
    return g
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def collate(self, samples):
        ...
        for idx, graph in enumerate(graphs):
            ...
            graphs[idx].edata['edge_attr'] = graph.edata['edge_attr'].float()
            graphs[idx].ndata['pe_attr'] = graph.ndata['pe_attr'].float()
    ...
    def _add_hop_neighbors(self, k, max_edge_attr_num, max_hop_num,
        ...
        """generate multi-hop neighbors for input PyG graph using shortest path distance kernel
           Args:
               ...
               kernel (str): kernel used to extract neighbors
           """
        self.train.graph_lists = [extract_multi_hop_neighbors(g, k, max_edge_attr_num, max_hop_num,
                                max_edge_type, max_edge_count, max_distance_count, kernel) for g in self.train.graph_lists]
        self.val.graph_lists = [extract_multi_hop_neighbors(g, k, max_edge_attr_num, max_hop_num,
                                max_edge_type, max_edge_count, max_distance_count, kernel) for g in self.val.graph_lists]
        self.test.graph_lists = [extract_multi_hop_neighbors(g, k, max_edge_attr_num, max_hop_num,
                                max_edge_type, max_edge_count, max_distance_count, kernel) for g in self.test.graph_lists]
